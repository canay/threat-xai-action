...
| Held-out policy-context robustness and manuscript figure | `code/08_policy_context_robustness.py --seed 42 --min-rule-rows 50 --n-estimators 300 --max-depth 6 --device cpu`; aggregate-only Figure 5 rendered by `code/21_render_heldout_context_audit.py` v2.0 with the manuscript validation-figure visual system | `results/policy_context_robustness/policy_context_rule_summary.csv`, `policy_context_heldout_aggregate.csv`, `policy_context_heldout_results.csv`, `policy_context_heldout_confusions.csv`, `policy_context_robustness_metadata.json`, `fig_heldout_policy_context_audit.png`, and its render metadata |
| Missing-rule encoding sensitivity and pooled held-out-context metrics | `code/22_reviewer_revision_sensitivity.py --data data/processed/threat_five_class.csv`; paired seed-42 XGBoost sensitivity plus pooling of saved aggregate confusion counts | `results/reviewer_revision_sensitivity/rule_missingness_sensitivity.csv`, `rule_missingness_sensitivity_confusions.csv`, `heldout_named_context_pooled_metrics.csv`, `heldout_named_context_pooled_classwise.csv`, and metadata |
| Raw session-identifier coverage feasibility | `code/23_session_identifier_coverage_audit.py --raw-data <controlled-raw.csv>`; writes aggregate coverage only | `results/reviewer_revision_sensitivity/session_identifier_coverage_audit.json`; no identifiers or event-level rows are stored |
| Rule-missingness regime and support-qualified context sensitivity | `code/24_rule_context_regime_audit.py --data data/processed/threat_five_class.csv`; aggregate-only profiling plus pooling of saved context confusion counts after the fixed 0.90 maximum class-support-removal filter | `results/reviewer_revision_sensitivity/rule_missingness_profile.csv`, `heldout_named_context_support_qualified_metrics.csv`, `heldout_named_context_support_qualified_classwise.csv`, and metadata |
...
| SHAP and LIME explanation artifacts | `code/04_xai_explanations.py`; canonical locked environment, with 8-point Lato Regular axis-title and 8-point Inter Regular non-bold axis-tick typography hash-recorded; the summary beeswarm uses constant-hue points for nominal encoded categories | `results/xai/xai_generation_summary.json`, `results/xai/xai_shap_global_importance.csv`, and the three manuscript-facing explanation figures; the one-time geometry-preserving neutral render is hash-recorded in `fig_xai_shap_summary.render.json`; row-level LIME tables remain controlled |
