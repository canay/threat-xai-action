...
| Fixed-model test-record bootstrap and chronological per-class recall | `code/11_q1_audit_revision_checks.py --data data/processed/threat_five_class.csv --confusions results/extensions/q1_validation_confusions.csv --fit-xgb --bootstrap-iters 1000` | `results/q1_audit_revision/xgb_fixed_model_bootstrap_ci.csv`, `results/q1_audit_revision/xgb_fixed_model_per_class_recall_ci.csv` |
...
| Source-port-excluded near-duplicate grouping | `code/14_sourceport_nearduplicate_ablation.py` | `results/q1_audit_revision/sourceport_nearduplicate_ablation.csv` |
...
| Public UCI schema-transferable instantiation | Download the CC BY 4.0 `log2.csv` from the official [UCI Internet Firewall Data page](https://archive.ics.uci.edu/dataset/542/internet+firewall+data) (DOI: [10.24432/C5131M](https://doi.org/10.24432/C5131M)), then run `code/16_uci_leaf_instantiation.py --data data/uci_internet_firewall/log2.csv` | `results/uci_leaf_instantiation/uci_leaf_results.json` |
...
| Class-distribution, combined holdout/CV, and feature-group manuscript figures | `code/19_render_manuscript_figures.py` with the aggregate processing manifest and canonical benchmark/strengthening CSV files | `results/manuscript_figures/fig_class_distribution.png`, `results/manuscript_figures/fig_results_ablation_cv_combined.png`, `results/manuscript_figures/fig_feature_group_validation.png`, `results/manuscript_figures/manuscript_figure_render_metadata.json` |
...
| Operational review queue and unseen-context audit | `code/09_operational_review_and_context_audit.py --n-estimators 300` | Aggregate-only `results/operational_review_context_audit/`; public per-context values use deterministic aliases and no event-level queue is included |
| SHAP and LIME explanation artifacts | `code/04_xai_explanations.py` | `results/xai/xai_generation_summary.json`, `results/xai/xai_shap_global_importance.csv`, and the three manuscript-facing explanation figures; row-level LIME tables remain controlled |
